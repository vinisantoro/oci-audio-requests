...
function getIdPConfig() {
  ...
  if (!idpSsoUrl && !idpMetadataUrl) {
    throw new Error(
      'OCI_IDP_SSO_URL (or SAML_IDP_SSO_URL) environment variable is required. ' +
      'Get this from OCI Console > Identity & Security > Identity > Identity Providers > [Your IdP] > Details. ' +
      'Example: https://idcs-xxx.identity.oraclecloud.com/fed/v1/idp/sso'
    );
  }
  
  // If only metadata URL is provided without SSO URL, we need SSO URL
  if (idpMetadataUrl && !idpSsoUrl) {
    throw new Error(
      'OCI_IDP_SSO_URL is required even if OCI_IDP_METADATA_URL is set. ' +
      'The metadata URL is for reference only - you must provide the SSO URL directly. ' +
      'Get SSO URL from: OCI Console > Identity Providers > [Your IdP] > Details > SSO URL'
    );
  }

  // Auto-derive SLO URL from SSO URL if not provided
  let derivedSloUrl = idpSloUrl;
  if (!derivedSloUrl && idpSsoUrl) {
    // Try common patterns for IDCS/Oracle Identity Cloud Service
    if (idpSsoUrl.includes('/fed/v1/idp/sso')) {
      derivedSloUrl = idpSsoUrl.replace('/idp/sso', '/idp/slo');
    } else if (idpSsoUrl.includes('/fed/v1/saml/sso')) {
      derivedSloUrl = idpSsoUrl.replace('/saml/sso', '/saml/slo');
    } else if (idpSsoUrl.includes('/sso')) {
      derivedSloUrl = idpSsoUrl.replace('/sso', '/slo');
    } else if (idpSsoUrl.includes('/saml2/idp/sso')) {
      derivedSloUrl = idpSsoUrl.replace('/saml2/idp/sso', '/saml2/idp/slo');
    }
  }

  const config = {
    sso_login_url: idpSsoUrl,
    sso_logout_url: derivedSloUrl,
    certificates: []
  };
